"""US market analyzer — SEC filings + FMP FCF + analysis tracker."""
...
class USAnalyzer(MarketAnalyzer):
    ...
    analyze_button_label = "📊 一键分析 (K线 + 财务)"
    data_source_desc = "FMP K线 + FMP FCF"
    supports_filing_download = True
    ...
    def _render_tracker_ui(self):
        ...
        # Cache this count — computing it requires n_analyzed filesystem calls.
        # Invalidate only when the number of analyzed tickers changes.
        _fcf_ver_key = "_us_fcf_count_ver"
        _fcf_cnt_key = "_us_fcf_count"
        if st.session_state.get(_fcf_ver_key) != n_analyzed:
            n_with_fcf = sum(1 for tk in analyzed if self._has_saved_fcf(tk))
            st.session_state[_fcf_cnt_key] = n_with_fcf
            st.session_state[_fcf_ver_key] = n_analyzed
        else:
            n_with_fcf = st.session_state[_fcf_cnt_key]
