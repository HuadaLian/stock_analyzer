...
#### 1b) `etl_us_bulk_state`：跳过、重试、过期再跑、局部刷新

| 参数 / 状态 | 含义 |
|-------------|------|
| 默认（无 `--force`） | `status=done` 的 ticker **跳过**整票 fetch+apply。 |
| `--force` | 无视 `done`/`skipped`，整票重跑；**OHLCV 仍按库内 `MAX(date)+1` 增量拉取**，不是从上市首日全量重下。 |
| `--retry-failed` | 仅重跑 `failed`。默认 **不重试** `last_error` 看起来像「仅 OHLCV」的失败（如短增量窗口下 `no price history`）；需要时用 **`--retry-failed-include-ohlcv`**。 |
| `--stale-days N`（`N>0`） | `done` 也允许再跑：当 `ohlcv_daily` / `fundamentals_annual` / `fmp_dcf_history` / `companies.updated_at` 中 **任一路**最新日期距今天 ≥ `N` 日时本票进入工作队列。`N=0`（默认）表示从不因「过期」重跑 `done`。 |
| `--refresh-mode` | `full`（默认）\|`ohlcv`\|`fundamentals`\|`fmp_dcf`：只拉/只写对应子集（见 `python -m etl.us_bulk_run --help`）。单票 `python etl_run.py --tickers …` 同样支持 `--refresh-mode`。 |
...
- **维度定义（单一事实来源）**：`data_quality_spec.py` 列出逐 ticker D1 就绪项与 US 市场/ETL 切片，含检测文字说明与对应代码位置。
- **一键跑全量并写缓存**：`python -m reports.run_db_quality_audit`（输出 `../reports/db_quality_cache/`）。Streamlit「数据库质量检测」页只读该缓存。
- **逐票 CLI**：`python -m db.checks --ticker NVDA` 或 `--all`。
