...
def _build_verification_fix_prompt(
    ...
    if (market or "").upper() == "US":
        market_currency_rule = (
            "4. For US annual reports (10-K/20-F), OCF/CapEx/FCF MUST be in USD.\n"
            "   Some companies (e.g. Chinese ADRs) report financials in RMB/CNY.\n"
            "   If the filing is in a non-USD currency, you MUST either:\n"
            "   (a) Use the USD convenience-translation table in the same filing, OR\n"
            "   (b) Apply the exchange rate stated in the filing to convert to USD.\n"
            "   If neither is available, return null for OCF/CapEx/FCF — do NOT return\n"
            "   non-USD values pretending they are USD.\n"
        )
...
def _build_batch_fcf_prompt(
    ...
    fmp_currency = (fmp_currency or "USD").upper()
    ...
    if (market or "").upper() == "US":
        if fmp_currency == "USD":
            market_currency_rule = (
                ...
                "- FMP data reference: Financial Modeling Prep confirms this company reports in USD.\n"
                "  Use USD values directly from the filing.\n"
                "- Some filings (e.g. Chinese ADRs like BABA) report in RMB/CNY.\n"
                "  If the filing is non-USD, use (a) the USD convenience-translation table\n"
                "  in the same filing, or (b) the exchange rate stated in the filing.\n"
                "- If neither is available, return null for those fields — NEVER output\n"
                "  non-USD values as if they were USD.\n"
            )
        else:
            market_currency_rule = (
                f"- FMP data reference: Financial Modeling Prep reports this company's financials"
                f" in {fmp_currency} (not USD).\n"
                f"  This means the company's primary reporting currency in its filings is {fmp_currency}.\n"
                f"- Please extract OCF and CapEx in {fmp_currency} as reported in the filing.\n"
                f"  → Look for the {fmp_currency} financial statements (NOT the USD convenience table).\n"
                f"  → Return values in {fmp_currency}; do NOT convert to USD.\n"
                f"- If only a USD table is available and no {fmp_currency} table exists, use USD\n"
                f"  and set \"currency_note\": \"USD (no {fmp_currency} table found)\" in the JSON.\n"
            )
...
def fill_fcf_table_with_llm(
    ...
    for batch_i, batch in enumerate(batches):
        ...
        if _is_cn:
            ...
            prompt = _build_batch_fcf_prompt(table_csv, batch_years, market=market, fmp_currency=fmp_currency)
    ...
    if failed_years:
        ...
        for year in failed_years:
            ...
            if _is_cn_retry:
                ...
                _retry_prompt = _build_batch_fcf_prompt(table_csv, [year], market=market, fmp_currency=fmp_currency)
